...
function dataStruct = createHetMeasure(masking, CElist, CSElist, outdir)
    ...
    spm_progress_bar('Init',5,'Heterogeneity','Current stage');
    ...
    voxelValues_P = -log10(chi2cdf(voxelValues_Q,lengthUseful-1, 'upper'));
    
    spm_progress_bar('Set',4);
    
    %Create the final maps
    finalMap_Q = repmat(nan, dim(1)*dim(2)*dim(3), 1);
    finalMap_Q(threshVec==1) = voxelValues_Q; 
    finalMap_Q = reshape(finalMap_Q, [dim(1), dim(2), dim(3)]);
    
    finalMap_I2 = repmat(nan, dim(1)*dim(2)*dim(3), 1);
    finalMap_I2(threshVec==1) = voxelValues_I2; 
    finalMap_I2 = reshape(finalMap_I2, [dim(1), dim(2), dim(3)]);
